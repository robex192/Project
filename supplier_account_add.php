...
include('../asset/includes/dbcon.php');
	...
	$query2=mysqli_query($con,"select * from Organization where organ_id='$id'")or die(mysqli_error());
		$rowc=mysqli_fetch_array($query2);
		$supplier_name=$rowc['organ_name'];
	
	if ($type=="Payment")
	  {
		$remarks="added a payment of $amount to $supplier_name";  
	   	mysqli_query($con,"UPDATE Organization SET balance=balance-'$amount' where organ_id='$id'") or die(mysqli_error()); 
